...
synthesis agree on every other leaf. Version 12 is separately reserved for the
protocol-12b mechanism change, so bumping here would have spent a tape
identity on a change that provably moves no generated byte and pushed the real
one to 13. This exemption does NOT generalize to any other fingerprint leaf:
