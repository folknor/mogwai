...
- **Cancel reasons never reach nautilus, and 0.64 can now carry them.**
  `OrderCanceled::new` gained a `reason` at 0.64 and reconciliation passes a
  report's `cancel_reason` through into it. The engine computes a reason for a
  market remainder ("insufficient displayed depth") in `mogwai-engine`'s
  `orders.rs` and only logs it: `VenueMessage::OrderCanceled` has no reason
  field, the adapter passes `None`, and its status reports never set
  `cancel_reason`. So the consumer-facing sentence below, that a market order
  "cancels the remainder with reason", describes a log line rather than
  anything a host can read. Carrying it is a protocol change.

- **`MogwaiTimer` no longer mirrors `LiveTimer` at the overflow edge.** 0.64
  made a `LiveTimer` whose next fire would overflow treat the current fire as
  its last. `MogwaiTimer` advances with `saturating_add`, so an unbounded timer
  near `u64::MAX` would refire at the same instant instead of expiring.
...
Moved to `notes/nautilus.md`, which is re-read at every nautilus bump rather
than with the daily backlog.
