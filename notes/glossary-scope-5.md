...
4. **[P2 CHANGED] The glossary's Divergence entry says generator arms are part
   ...
   found the fork missing in `mogwai-server`; the fork is not missing, it was
...
5. **[P2 REFUTED] `TAPE_PROTOCOL_VERSION` governs two generation origins, but
   ...
   writes a bounded CSV dump. `mogwai-server::source` constructs only
...
6. **`mogwai_lab::ledger` is not a ledger.** The glossary defines Ledger as one
   ...
   are reachable from a crate that depends on `mogwai-server`.
...
there is no bid, no ask and no top of book, and `mogwai-server`'s `/quotes`
