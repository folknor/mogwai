...
`TAPE_PROTOCOL_VERSION` bump from 12 to 13, which pushed the protocol-12b
mechanism landing to 14 at the time (`bugs-data.md` round 1 has since taken 14
for the calendar-aware `ReopenGap` crossing repair, so the reservation is now
15). Reviewing that pass turned up three more defects, fixed
