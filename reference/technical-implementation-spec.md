...
5. **Verification per brick.** Every change names its gate, matched to what
   ...
   test, or launching the server (`brokkr run -p mogwai-server`) and running
   `python3 scripts/smoke.py` for the live end-to-end path. If no command exists that can verify a gate
