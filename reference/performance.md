...
latency bounded by the deadline, and nothing has measured that under a real
mass-attach boot storm because nobody has run one. `MAX_QUEUED_HISTORY_REQUESTS` is the
...
shows up here as roughly 3 ms. Re-scoping the reading window was at this date
the one named lever that would move this number materially, unapplied because
it moves the estimator's identity and re-blesses the fill golden. The
2026-08-23 resident-window section below is what closed the number instead,
...
## 2026-08-23 the resident vol window closes the submit-budget debt

Host `bygg`, release, from the extended latency instrument
`read_market_latency_stays_within_submit_budget` (100 reads per tier, one per
memo bucket). The acceptance-time market reading's miss path had sat at
9.8 ms against the 5 ms submit budget since the checkpoint stride repair,
with the 300 s `VOL_WINDOW_NS` tape walk as the whole remaining cost and
re-scoping the window rejected because it moves the estimator's identity.

The closure is the third route: the same quantity, cheap. Each boat's tape
thread now keeps a resident trailing window of the prints it was already
producing (`mogwai-venue`'s `vol_window`, the `extremes` channel's sibling),
and a cache miss folds resident prints through the identical shared
arithmetic (`mogwai_data::vol_reading_from_trades`) instead of regenerating
the tape from a checkpoint. The walk survives as the fallback wherever the
window cannot prove it would match - a cold boat's first 300 simulated
seconds, a bucket the pacing has not proven, `speed = 0.0`, and any span the
walk's own budget might refuse - so every divergence route collapses back to
the walk, and no generated byte, band or fill moves: no
`TAPE_PROTOCOL_VERSION` bump, no re-bless. The bit-identity of the two paths
is pinned by `a_window_read_is_bit_identical_to_the_walk` (hand-built stream,
boundary instants included) and
`a_resident_window_serves_a_miss_walk_free_and_walk_identical` (real river
water through the production cache).

| path | 2026-08-14 | this run |
|---|---:|---:|
| miss served resident, median | (did not exist) | 208 us |
| miss served resident, p99 | (did not exist) | 448 us |
| miss falling back to the walk, median | 9.782 ms | 8.221 ms |
| miss falling back to the walk, p99 | 9.987 ms | 9.573 ms |
| memo hit | 0.096 ms | 0.081 ms |

The instrument now gates the hit and the resident miss against the original
5 ms budget, and keeps the walk pinned at its measured ceilings (25 ms median,
