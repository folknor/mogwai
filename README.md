...
committed fingerprint fitted offline to real trade archives - Binance
trade-level data for the crypto preset, delivered CME TBBO for the futures ones
(the running venue opens no CSV) - and injects the messy, realistic execution divergences - partial
...
What a forward run against it proves is execution robustness, never edge: the
edge belongs to a backtest on real history, and dollars earned on a synthetic
tape are a statement about the fitted distribution of worlds rather than about
next month's market. One seed is one path; a claim wants many.

> **Not ready for production.** mogwai is young and moving fast. The wire
> protocol, the config schema, the CLI and the generated tape all change
> without ceremony, and a tape identity is bumped rather than preserved, so an
> upgrade can redraw the river under you. Several parts of the venue are
> honest about being unfinished: only three instrument presets exist, two of
> which share one fit, and the equity, perpetual and inverse classes are served
> the default shape rather than a fitted one. Pin a commit if you need a result
> to reproduce, and read what a forward run against a synthetic tape does and
> does not prove before you trust a number that comes out of it.

![Four days of one-minute bars generated by mogwai](docs/example-generated-bars.png)

Four days of the synthetic river, charted straight from the generator with no
...
The seed is the whole of the reproduction: that command draws the same river on
...
mogwai gen --help                  # dump a river offline, no venue involved
...
gap, because `0.63.0` means `>=0.63.0, <0.64.0`.
