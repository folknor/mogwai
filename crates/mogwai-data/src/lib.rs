...
pub use generated::{
    ...
    SIZE_LAW_ENTRIES, ScalarDiagnostic, ScalarError, SelfExcitingParams, SessionCalendar,
    SessionEnvelope, SessionProfile, SessionProfileError, ShotNoiseParams, SizeGrid, Struck,
    SweepShape, TickTraversal, TopOfBookSizes, TradeDisplacement, Trichotomy, VolTrace,
    WalkOutcome, WallMmppParams, WeeklyWindow, book_mid_ticks, ladder_level_units, place_book,
};
...
/// every checkpoint of that river carries it. Surged tapes always move: the same
...
impl MergeSource {
    ...
    pub fn starting_at(sources: Vec<Box<dyn TickSource>>, start_ts: Option<u64>) -> Self {
        ...
        // only after the whole pass. The loop used to `break` on the first fault,
...
mod tests {
    ...
    /// The merge's tie rule is documented explicitly, so it is pinned rather than left to
