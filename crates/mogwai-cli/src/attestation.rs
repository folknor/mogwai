...
//!
//! `mogwai_lab::tree`'s `TreeOracle` seam is compiled out of a production
...
pub fn refuse_scripted_tree_attestation() -> Result<()> {
    if !mogwai_lab::tree::tree_readings_are_production() {
        bail!(
...
mod tests {
    ...
    use mogwai_lab::tree::{ScriptedTree, install_tree_oracle};
